...
Light::Light(sf::Vector2f position, float intensity, float radius, int quality, sf::Color color)
 : m_position(position), m_intensity(intensity), m_influence_radius(radius), m_color(color), m_quality(quality)
{
}
...
void Light::AddTriangle(sf::Vector2f pt1,sf::Vector2f pt2, int minimum_wall,std::vector <Wall> &m_wall)
{
    ...
    {
        // l1 et l2 sont les positions relatives au centre de la lumiere des deux extremites du mur
        ...
        if(l1.x * l1.x + l1.y * l1.y < m_influence_radius * m_influence_radius)
        {
        ...
        if(l2.x * l2.x + l2.y * l2.y < m_influence_radius * m_influence_radius)
        {
    ...
    // Variable qui contiendra l'intensite calculee, pour le degrade
    float intensity;

    // On ajoute un shape
    m_shape.push_back(sf::Shape ());

    // On lui donne comme point de depart (0,0), le centre de la lumiere, avec la couleur et intensite maximal
    ...
    // Idem
    intensity=m_intensity-sqrt(pt2.x*pt2.x + pt2.y*pt2.y)*m_intensity/m_influence_radius;
...
void Light::Generate(std::vector <Wall> &m_wall)
{
    ...
    // buf est l'angle de chaque triangle, c'est donc 2pi divise par le nombre de triangles
    float buf=(M_PI*2)/(float)m_quality;

    // On ajoute tous les triangles qui composent la lumiere
    for(int i=0;i<m_quality;i++)
    {
        AddTriangle(sf::Vector2f((float)((float)m_influence_radius*cos((float)i*buf))
                                ,(float)((float)m_influence_radius*sin((float)i*buf))) ,
                    sf::Vector2f((float)((float)m_influence_radius*cos((float)(i+1)*buf))
                                ,(float)((float)m_influence_radius*sin((float)(i+1)*buf))),0,m_wall);
    }
}


// Differentes fonctions pour modifier les attributs de la lumiere, et pour les recuperer
...
float Light::GetRadius(){ return m_influence_radius; }
